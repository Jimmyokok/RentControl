...
namespace RentControl.Systems
{
    ...
    public class CustomPropertyRenterSystem : GameSystemBase, IDefaultSerializable, ISerializable, IPostDeserialize
    {
        ...
        public static float GetUpkeepExponent(Game.Zones.AreaType type)
        {
        ...
        // Token: 0x060055A8 RID: 21928 RVA: 0x003B2AC4 File Offset: 0x003B0CC4
        public static int GetUpkeep(int level, float baseUpkeep, int lotSize, AreaType areaType, bool isStorage = false)
        {
            if (areaType == AreaType.Residential)
            {
                return Mathf.RoundToInt(math.pow((float)level, CustomPropertyRenterSystem.kResidentialUpkeepExponent) * baseUpkeep * (float)lotSize);
            }
            return Mathf.RoundToInt(math.pow((float)level, CustomPropertyRenterSystem.GetUpkeepExponent(areaType)) * baseUpkeep * (float)lotSize * (isStorage ? 0.5f : 1f));
        }
        ...
        // Token: 0x060055AB RID: 21931 RVA: 0x003B2B14 File Offset: 0x003B0D14
        ...
        // Token: 0x060055AE RID: 21934 RVA: 0x003B2B58 File Offset: 0x003B0D58
        [Preserve]
        protected override void OnCreate()
        {
            ...
            RentUpgradeFactor = Plugin.RentUpgradeFactor.Value;
            RentUpgradeFactor = RentUpgradeFactor > 0 ? RentUpgradeFactor : 0;
            RentUpgradeFactor = RentUpgradeFactor < 100 ? RentUpgradeFactor : 100;
        ...
        // Token: 0x060055B0 RID: 21936 RVA: 0x003B2DD8 File Offset: 0x003B0FD8
        ...
        // Token: 0x060055B1 RID: 21937 RVA: 0x003B2E8C File Offset: 0x003B108C
        [Preserve]
        protected override void OnUpdate()
        {
            ...
            jobData2.m_Lots = this.__TypeHandle.__Game_Areas_Lot_RO_ComponentLookup;
            jobData2.m_Geometries = this.__TypeHandle.__Game_Areas_Geometry_RO_ComponentLookup;
            jobData2.m_Attached = this.__TypeHandle.__Game_Objects_Attached_RO_ComponentLookup;
            jobData2.m_LandValues = this.__TypeHandle.__Game_Net_LandValue_RO_ComponentLookup;
            jobData2.m_ConsumptionDatas = this.__TypeHandle.__Game_Prefabs_ConsumptionData_RO_ComponentLookup;
            jobData2.m_BuildingType = this.__TypeHandle.__Game_Buildings_Building_RO_ComponentTypeHandle;
            jobData2.m_BuildingDatas = this.__TypeHandle.__Game_Prefabs_BuildingData_RO_ComponentLookup;
        ...
        // Token: 0x060055B2 RID: 21938 RVA: 0x003B3A78 File Offset: 0x003B1C78
        ...
        // Token: 0x060055B3 RID: 21939 RVA: 0x003B3AF8 File Offset: 0x003B1CF8
        public void DebugLevelDown(Entity building, ComponentLookup<BuildingCondition> conditions, ComponentLookup<SpawnableBuildingData> spawnables, ComponentLookup<PrefabRef> prefabRefs, ComponentLookup<ZoneData> zoneDatas, ComponentLookup<BuildingPropertyData> propertyDatas)
        {
            if (conditions.HasComponent(building) && prefabRefs.HasComponent(building))
            {
                ...
                if (spawnables.HasComponent(prefab) && propertyDatas.HasComponent(prefab))
                {
                    ...
                    if (zoneDatas.HasComponent(spawnableBuildingData.m_ZonePrefab))
                    {
                        int levelingCost = BuildingUtils.GetLevelingCost(zoneDatas[spawnableBuildingData.m_ZonePrefab].m_AreaType, propertyDatas[prefab], (int)spawnableBuildingData.m_Level, base.EntityManager.GetBuffer<CityModifier>(this.m_CitySystem.City, true));
        ...
        private static float RentUpgradeFactor;
        ...
        {
            // Token: 0x060055B8 RID: 21944 RVA: 0x003B3BC8 File Offset: 0x003B1DC8
            public void Execute()
            {
                Entity entity;
        ...
        {
            // Token: 0x060055B9 RID: 21945 RVA: 0x003B3F38 File Offset: 0x003B2138
            public void Execute()
            {
            ...
            // Token: 0x060055BA RID: 21946 RVA: 0x003B416C File Offset: 0x003B236C
            ...
            // Token: 0x060055BB RID: 21947 RVA: 0x003B42FC File Offset: 0x003B24FC
            ...
            private struct Iterator : INativeQuadTreeIterator<Entity, Bounds2>, IUnsafeQuadTreeIterator<Entity, Bounds2>
            {
                ...
                // Token: 0x060055BD RID: 21949 RVA: 0x003B448C File Offset: 0x003B268C
        ...
        {
            // Token: 0x060055BE RID: 21950 RVA: 0x003B45F8 File Offset: 0x003B27F8
            public void Execute()
            {
        ...
        {
            // Token: 0x060055BF RID: 21951 RVA: 0x003B4648 File Offset: 0x003B2848
        ...
        {
            // Token: 0x060055C1 RID: 21953 RVA: 0x003B48F8 File Offset: 0x003B2AF8
            public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
            {
                if (chunk.GetSharedComponent<UpdateFrame>(this.m_UpdateFrameType).m_Index != this.m_UpdateFrameIndex)
                {
                ...
                for (int i = 0; i < chunk.Count; i++)
                {
                    ...
                    if (this.m_SpawnableBuildingData.HasComponent(prefab))
                    {
                        ...
                        int levelingCost = BuildingUtils.GetLevelingCost(areaType, buildingPropertyData, (int)spawnableBuildingData.m_Level, cityEffects);
                        int num = (spawnableBuildingData.m_Level == 5) ? BuildingUtils.GetLevelingCost(areaType, buildingPropertyData, 4, cityEffects) : levelingCost;
                        ...
                        for (int j = 0; j < dynamicBuffer.Length; j++)
                        {
                            ...
                            if (this.m_PropertyRenters.HasComponent(renter))
                            {
                                ...
                                if (this.m_Storages.HasComponent(renter))
                                {
                                    ...
                                    rent_paid = num3;
                                    num3 = num3 > 0 ? MathUtils.RoundToIntRandom(ref random, (float)num3 * RentUpgradeFactor) : MathUtils.RoundToIntRandom(ref random, (float)num3 * RentPaymentFactor);
                                }
                                else
                                {
                                    int areaSize = buildingData.m_LotSize.x * buildingData.m_LotSize.y;
                                    Attached attached;
                                    DynamicBuffer<Game.Areas.SubArea> subAreas;
                                    if (this.m_Attached.TryGetComponent(nativeArray[i], out attached) && this.m_SubAreas.TryGetBuffer(attached.m_Parent, out subAreas))
                                    {
                                        areaSize += Mathf.RoundToInt(0.5f * ExtractorAISystem.GetArea(subAreas, this.m_Lots, this.m_Geometries) / 64f);
                                    }
                                    Entity roadEdge = nativeArray4[i].m_RoadEdge;
                                    float landvalue = 0f;
                                    if (this.m_LandValues.HasComponent(roadEdge))
                                    {
                                        landvalue = (float)areaSize * this.m_LandValues[roadEdge].m_LandValue;
                                    }
                                    num3 = MathUtils.RoundToIntRandom(ref random, (RentUpgradeFactor * (float)RentAdjustSystem.GetRent(consumptionData, buildingProperties, landvalue, areaType).x) / (float)CustomPropertyRenterSystem.kUpdatesPerDay);
                                    rent_paid = MathUtils.RoundToIntRandom(ref random, (float)propertyRenter.m_Rent / (float)CustomPropertyRenterSystem.kUpdatesPerDay);
                                    num2 += num3;
                                }
                                EconomyUtils.AddResources(Resource.Money, -rent_paid, this.m_Resources[renter]);
                                if (nativeArray3.Length > 0)
                                {
                                    ...
                                    {
                                        int FAKE_MAX = (1 << 29) - 1;
                                        if(FAKE_MAX - num3 <= buildingCondition.m_Condition)
                                        {
                                            buildingCondition.m_Condition = FAKE_MAX;
                                        }
                                        else
                                        {
        ...
        {
            // Token: 0x060055C3 RID: 21955 RVA: 0x003B4D08 File Offset: 0x003B2F08
            public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
            {
                NativeArray<Entity> nativeArray = chunk.GetNativeArray(this.m_EntityType);
                for (int i = 0; i < nativeArray.Length; i++)
        ...
        {
            // Token: 0x060055C5 RID: 21957 RVA: 0x003B4D88 File Offset: 0x003B2F88
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
